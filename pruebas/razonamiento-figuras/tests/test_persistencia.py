...
from comun import URL, SERIES, iniciar, terminar_parte, mantener, reloj, esperar
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        reloj_antes = await pg.inner_text("#timer")
        for v in (3, 1, 2):
            await pg.click(f'[data-act=pick][data-ctx=real][data-v="{v}"]')
            await esperar(pg, 400)
        await pg.reload()
        await pg.wait_for_timeout(300)
        comprobar(await pg.inner_text(".qnum") == "Pregunta 4 de 12", "no reanudó en la pregunta 4")
        comprobar(await pg.inner_text("#timer") <= reloj_antes, "recargar dio tiempo extra")
        ...
        await pg.evaluate("localStorage.setItem('rfFormaA.v1','{dañado')")
        await pg.reload()
        await pg.wait_for_timeout(300)
        comprobar(await pg.inner_text(".qnum") == "Pregunta 3 de 12", "el respaldo no recuperó el avance")
