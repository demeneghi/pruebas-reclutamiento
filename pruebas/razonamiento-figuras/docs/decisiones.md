...
- Serie A: la pieza correcta es exactamente la región del hueco. Los distractores son la misma textura girada, volteada o tomada de otra parte del dibujo donde cambia (separación, tamaño, curvatura), otra textura o la pieza vacía. Nunca la misma textura corrida: en un patrón que se repite, la pieza corrida solo se distingue por el corte en el borde y es una segunda respuesta defendible. `tests/test_reactivos.py` lo verifica alineando cada distractor con la respuesta.
